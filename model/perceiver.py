...
class Perceiver(nn.Module):
    def __init__(
        ...
        *,
        ...
        max_freq,
        freq_base = 2,
        input_channels = 3, # 768
        input_axis = 2,
        num_latents = 512, # 384
        latent_dim = 512,  # 768
        cross_heads = 1, # 8
        latent_heads = 8,
        cross_dim_head = 64, # 96
        latent_dim_head = 64, # 96
        num_classes = 1000,
        attn_dropout = 0.,
        ff_dropout = 0.,
...
class trans_ffn1(nn.Module):
    ...
    def forward(self, data, mask = None):
        ...
        x1 = self.layers1(data) + data
        x1 = self.layers2(x1) + x1

        return x1


class trans_ffn2(nn.Module):
    def __init__(
        self,
        input_channels = 768,
        num_tokens = 128,
...
if __name__ == '__main__':
    model = Occlusion_detector()
    x = torch.randn(3, 128, 768)
    out = model(x)
    print(out.size())
