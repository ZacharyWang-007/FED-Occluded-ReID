...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        if self.local_feature:
            for blk in self.blocks[:-1]:   # discard the final classification token
                x = blk(x)
            return x

        else:
            for blk in self.blocks:    # include the final classification token
                x = blk(x)

            x = self.norm(x)  # so, how's here. 
            
            # return x[:, 1:]
            # x_spatial = x[:, 1 :  , :].reshape(x.size(0), 16, 8, x.size(2))
            # x_spatial = x_spatial.transpose(2, 3).transpose(1, 2)

            # x_spatial = x[:, 1:].permute(0, 2, 1).view(x.size(0), self.embed_dim, 16, 8)
